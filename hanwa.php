...
<head>
    ...
    <style>
        ...
        /* Efek magnetic — scale saat hover, dikelola JS juga */
        ...
        /* ============================================================
                   2. SLIDE-IN CONTENT (iframe entrance)
                   ============================================================ */
        #main-frame {
            width: 100%;
            height: 100%;
        ...
        /* ============================================================
                   3. PROGRESS INDICATOR (Line Loader)
                   ============================================================ */
        #progress-line {
            position: absolute;
            top: 0;
        ...
        @media (min-width: 769px) {
            ...
            /* Tooltip container agar tetap di dalam flow */
        ...
        /* ============================================================
                   EXTRA:  magnetic "pull" effect via JS
                   ============================================================ */
...
<body class="flex p-3 md:p-4 gap-4 h-screen w-screen overflow-hidden">
    ...
    <aside class="sidebar rounded-2xl flex flex-col h-full border border-slate-200" id="sidebar">
        ...
        <nav class="nav-menu flex flex-col gap-1.5 px-3 py-4 overflow-y-auto flex-1">
            ...
            <!-- ===== MENU ITEMS dengan data atribut untuk tooltip ===== -->
...
<main class="flex-1 flex flex-col gap-4 min-w-0 relative">
    ...
    <div class="flex-1 bg-white rounded-2xl overflow-hidden shadow-sm border border-slate-200 relative bg-slate-50/30">
        ...
        <!-- PROGRESS LINE (Subtle Indicator) -->
    ...
    (function() {
        ...
        navLinks.forEach(link => {
            ...
            link.addEventListener('mousemove', function(e) {
                ...
                // gerakan magnetik halus (translate)
                const moveX = dx * 3.5;
                const moveY = dy * 3.5;

                // terapkan via CSS custom property untuk glow
                this.style.setProperty('--mx', (x / rect.width * 100) + '%');
                this.style.setProperty('--my', (y / rect.height * 100) + '%');

                // transform utama: scale + translate kecil
                if (!this.classList.contains('active')) {
                    this.style.transform =
                        `translate(${moveX}px, ${moveY}px) scale(1.04)`;
                } else {
                    // active tetap lebih kuat
                    this.style.transform =
        ...
        // ============================================================
        // 2. SMART FLOATING TOOLTIP (collapsed mode)
        // ============================================================
        ...
        function updateTooltipPosition(tooltip, link) {
            ...
            // posisi di samping kanan ikon
            let left = rect.right + 12;
            let top = rect.top + rect.height / 2 - tooltipRect.height / 2;

            // biar tidak keluar layar
        ...
        // Pada collapsed mode, tooltip muncul saat hover
        navLinks.forEach(link => {
            ...
            link.addEventListener('mouseenter', function(e) {
                ...
                // pastikan tooltip di posisi yang benar
        ...
        // ============================================================
        // 3. PROGRESS INDICATOR + SLIDE-IN CONTENT
        // ============================================================
        function startProgress() {
            ...
            // animasi progress ke 100% secara bertahap
            let p = 0;
            const step = () => {
                if (p >= 100) {
                ...
                // increment non-linear: makin cepat di awal, pelan di akhir
        ...
        // ============================================================
        // 4. IFRAME LOAD HANDLER (Slide-in + Progress)
        // ============================================================
        iframe.addEventListener('load', function() {
            // Selesai loading → slide-in
            ...
            // Sembunyikan header & sidebar bawaan iframe (jika same-origin)
            try {
        ...
        // ============================================================
        // 5. NAVIGASI TAB (dengan progress & slide-in)
        // ============================================================
        function switchTab(e, link) {
            e.preventDefault();
            const targetUrl = link.getAttribute('href');

            // Cegah reload jika sama
            ...
            // Hilangkan slide-in state (iframe akan fade-out)
            ...
            headerBox.className = `${link.dataset.bg} ${link.dataset.text} p-2.5 rounded-xl shadow-sm transition-all duration-300`;
            document.getElementById('title-dot').className =
                `w-1.5 h-1.5 rounded-full ${link.dataset.text.replace('text', 'bg')}`;

            // Set iframe src & simpan history
        ...
        if (initialLink) {
            // set active state tanpa trigger reload ganda
            ...
            hb.className = `${initialLink.dataset.bg} ${initialLink.dataset.text} p-2.5 rounded-xl shadow-sm transition-all duration-300`;
            document.getElementById('title-dot').className =
                `w-1.5 h-1.5 rounded-full ${initialLink.dataset.text.replace('text', 'bg')}`;

            iframe.src = savedUrl;
            // progress akan jalan saat iframe load
            startProgress();
        } else {
            // default: aktifkan yang pertama
        ...
        // ============================================================
        // 8. RESIZE: update tooltip positions
        // ============================================================
        let resizeTimer;
        ...
        // ============================================================
        // 9. FALLBACK: jika iframe tidak kunjung load (5 detik)
        // ============================================================
        ...
        console.log('✨ ReqraWA Dashboard — Efek: Magnetic, Slide-In, Progress Line, Smart Tooltip');
    })();
</script>
</body>
</html>