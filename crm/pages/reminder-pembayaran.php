...
<div class="reminder-payment-grid">
    <section class="reminder-card reminder-target-card">
        ...
        <form class="reminder-filters" method="get">
            ...
            <label><span>Halaqoh</span><select name="halaqoh" multiple size="4"><?php foreach ($halaqohList as $item): ?><option value="<?= htmlspecialchars($item) ?>" <?= in_array($item, $halaqoh, true)?'selected':'' ?>><?= htmlspecialchars($item) ?></option><?php endforeach; ?></select></label>
