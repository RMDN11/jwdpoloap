...
<body class="flex flex-col h-screen overflow-hidden bg-slate-50 relative">
                                    ...
                                    <tr class="row-animate group">
                                        ...
                                        <td class="p-4 text-right align-top">
                                            <div class="flex justify-end gap-1.5">
                                                <!-- Tombol Copy -->
                                                <button type="button" class="bg-white border border-slate-200 text-emerald-500 p-2 rounded hover:bg-emerald-50 hover:border-emerald-200 transition-colors shadow-sm copy-btn" 
                                                        data-content="<?= htmlspecialchars($tmpl['content'], ENT_QUOTES) ?>" title="Salin Template">
    ...
    <script>
        ...
        // Fitur Copy Template ke Clipboard
            document.querySelectorAll('.copy-btn').forEach(btn => {
                btn.addEventListener('click', function() {
                    const contentToCopy = this.dataset.content;
                    
                    navigator.clipboard.writeText(contentToCopy).then(() => {
                        // Memanggil fungsi showToast yang sudah ada di file Anda
                        showToast("Template berhasil disalin!", "success");
                    }).catch(err => {
                        showToast("Gagal menyalin template.", "error");
                        console.error("Kesalahan clipboard: ", err);
                    });
                });
            });
