...
// Fungsi kirimPesan tetap dipertahankan jika sewaktu-waktu dibutuhkan untuk tes manual
...
// ============================================
// PROSES AJAX REQUEST (SISTEM ANTREAN / QUEUE)
// ============================================
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['ajax_add_to_queue'])) {
    ...
    if (empty($templatePesan)) {
        echo json_encode(['status' => 'error', 'msg' => 'Template pesan kosong!']); 
        exit;
    }
    ...
    // Jika Mode Select All Filtered: Ambil langsung dari Database
    if ($isAllFiltered) {
        ...
        // Jika mode ceklis manual: Ambil dari array POST
        $selectedPeserta = $_POST['selected_peserta'] ?? [];
    }
    
    // PROSES INSERT KE TABEL ANTREAN (wa_queue_satuan)
    $countSuccess = 0;
    ...
    if ($stmt_queue) {
        foreach ($selectedPeserta as $p_info) {
            ...
            $stmt_queue->bind_param("sss", $nowa, $namaPeserta, $pesanBody);
            $stmt_queue->execute();
            $countSuccess++;
        }
        $stmt_queue->close();
    ...
    echo json_encode(['status' => 'success', 'msg' => "$countSuccess pesan berhasil dimasukkan ke antrean."]);
    exit;
}
...
// PROSES HAPUS SEMUA LOG (Diubah untuk membersihkan riwayat Antrean juga opsional)
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_all_logs'])) {
    if ($conn->query("TRUNCATE TABLE wa_queue_satuan") && $conn->query("TRUNCATE TABLE log_wa")) {
        $_SESSION['notification'] = "Semua riwayat log dan antrean berhasil dihapus.";
        $_SESSION['notification_type'] = 'success';
    } else {
...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['delete_log_id'])) {
    ...
    $stmt = $conn->prepare("DELETE FROM wa_queue_satuan WHERE id = ?"); // Menghapus dari queue table sebagai log
...
// ============================================
// AMBIL DATA LOG DARI ANTREAN
// ============================================
$logPesan = [];
// Mengambil log langsung dari tabel antrean untuk melihat status terkini
$logResult = $conn->query("SELECT id, nama, pesan as message, status, created_at, sent_at FROM wa_queue_satuan ORDER BY id DESC LIMIT 20");
if ($logResult) { 
...
<header class="bg-gray-800 text-white shadow-sm sticky top-0 z-10">
            ...
            <div id="loader" class="hidden animate-fade-in-down mb-6 bg-white rounded-xl shadow-lg border border-blue-100 overflow-hidden relative">
                ...
                <div class="p-5 flex flex-col sm:flex-row items-center gap-5">
                    <div class="bg-blue-50 w-14 h-14 rounded-full flex items-center justify-center shrink-0 shadow-inner border border-blue-100">
                        <i class="fas fa-server text-2xl text-blue-500 animate-pulse"></i>
                    </div>
                    <div class="flex-1 w-full">
                        <div class="flex justify-between items-end mb-2">
                            <div>
                                <h3 class="font-extrabold text-slate-800 text-base">Memproses ke Antrean Server...</h3>
                                <p id="progressStatus" class="text-xs font-medium text-slate-500 mt-0.5">Mempersiapkan ribuan data Anda dengan aman...</p>
                            </div>
                        </div>
                ...
                <div class="space-y-6 order-1 lg:order-2">
                    <div class="bg-card shadow-md rounded-2xl p-6 sticky top-24 border border-card">
                        ...
                        <button type="button" id="btn-kirim-reminder"
                            ...
                            <i class="fas fa-paper-plane text-2xl mr-2"></i> Kirim ke Antrean
                        </button>
                        ...
                        <div class="flex justify-between items-center mb-4">
                            <h2 class="text-lg font-semibold flex items-center text-primary">
                                <i class="fas fa-list-ol text-xl mr-2 text-accent"></i> Status Antrean
                            </h2>
                            <form method="POST" action="">
                                <button type="submit" name="delete_all_logs" value="1" 
                                        onclick="return confirm('Anda yakin ingin menghapus SEMUA riwayat antrean dan log?');" 
                        ...
                        <div class="space-y-2 max-h-80 overflow-y-auto pr-2">
                            <?php if(!empty($logPesan)): ?>
                                    ...
                                    <div class="relative flex items-center justify-between p-2 bg-gray-50 rounded-lg border border-gray-200 group">
                                        <div class="flex items-center space-x-3 overflow-hidden">
                                            <?php if ($log['status'] == 'sent'): ?>
                                                <i class="fas fa-check-circle text-green-500 text-xl" title="Terkirim"></i>
                                            <?php elseif ($log['status'] == 'failed'): ?>
                                                <i class="fas fa-times-circle text-red-500 text-xl" title="Gagal"></i>
                                            <?php else: ?>
                                                <i class="fas fa-clock text-yellow-500 text-xl" title="Menunggu Antrean (Pending)"></i>
                                            <?php endif; ?>
                                            <div>
                                                <p class="font-semibold text-gray-800 text-sm truncate"><?= htmlspecialchars($log['nama']) ?></p>
                                                <p class="text-gray-400 text-[10px]">
                                                    Dibuat: <?= date('d M, H:i', strtotime($log['created_at'])) ?>
                                                    <?= $log['sent_at'] ? '| Dikirim: '.date('H:i', strtotime($log['sent_at'])) : '' ?>
                                                </p>
                                            </div>
                                        </div>
                                ...
                                <?php endforeach; ?>
                            <?php else: ?>
                                <p class="text-sm text-gray-500 text-center p-4">Antrean masih kosong.</p>
                            <?php endif; ?>
                        </div>
                    </div>
                </div>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // ---------------------------------------------------------
            // FUNGSI AJAX MEMASUKKAN KE ANTREAN (SATU KALI KLIK, CEPAT)
            // ---------------------------------------------------------
            ...
            btnKirimReminder.addEventListener('click', async function(e) {
                ...
                btnKirimReminder.innerHTML = '<i class="fas fa-spinner fa-spin text-2xl mr-2"></i> Memproses Antrean...';

                pStat.innerHTML = `Mengirim data ke database antrean...`;
                pBar.style.width = '50%';
                ...
                try {
                    ...
                    if(json.status === 'success') {
                        ...
                        pStat.innerHTML = `<span class="text-emerald-600 font-bold">Sukses!</span> ${json.msg} Cron Job akan mengirim perlahan. Memuat ulang halaman...`;
                    ...
                    alert("Terjadi kesalahan sistem saat menyimpan ke antrean.");
                    window.location.reload();
                }
            });
            ...
            function updateSelectionDisplay() {
                ...
                if (selectedCountInfo) selectedCountInfo.textContent = checkedCount;
